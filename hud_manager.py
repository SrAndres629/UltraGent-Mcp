...
Implementa:
- Generación dinámica de HUD.md
- Throttling de escrituras (1s mínimo)
- Agregación de estados de todos los módulos
- Human-in-the-loop signals
- Jerarquía visual con <details>
"""
...
HUD_TEMPLATE = """# 🎛️ ULTRAGENT HUD v2.0
...
class HUDManager:
    ...
    Recopila datos de todos los módulos y genera el HUD.md
    ...
    def __init__(self):
        ...
        self._mission_goal = "Sistema en espera de misión"
    ...
    def set_mission_goal(self, goal: str) -> None:
        """Define el objetivo global de la misión."""
        with self._lock:
    ...
    def request_human_decision(
        ...
        """Establece una señal de Human-in-the-loop."""
        with self._lock:
    ...
    def clear_human_signal(self) -> None:
        """Limpia la señal de decisión humana."""
        with self._lock:
    ...
    def _get_mcp_status(self) -> dict:
        ...
        try:
            ...
            return {"status": "🔴 Error", "version": "?", "tools": 0}
    ...
    def _get_sentinel_status(self) -> dict:
        ...
        try:
            ...
            return {
                "status": "🟢 Watching" if status.get("watching") else "🟡 Idle",
                "events": status.get("stats", {}).get("events_detected", 0),
            }
        except Exception:
            return {"status": "⚫ N/A", "events": 0}
    ...
    def _get_router_status(self) -> dict:
        ...
        try:
            ...
            return {"status": "⚫ N/A", "tokens": "0", "details": {}}
    ...
    def _get_scout_status(self) -> dict:
        ...
        try:
            ...
            return {
                "status": "🟢 Ready" if status.get("has_token") else "🟡 No Token",
                "searches": stats.get("searches", 0),
                "details": stats,
            }
        except Exception:
            return {"status": "⚫ N/A", "searches": 0, "details": {}}
    ...
    def _get_mechanic_status(self) -> dict:
        ...
        try:
            ...
            stats = status.get("stats", {})
            return {
                "status": "🟢 Ready" if status.get("docker_available") else "🟡 No Docker",
                "docker": "Available" if status.get("docker_available") else "Offline",
                "details": stats,
            }
        except Exception:
            return {"status": "⚫ N/A", "docker": "Unknown", "details": {}}
    ...
    def _get_vision_status(self) -> dict:
        ...
        try:
            ...
            return {"status": "⚫ N/A", "scans": 0}
    
    def _get_sentinel_alerts(self) -> str:
        """Obtiene últimas alertas del Sentinel."""
        try:
    ...
    def _get_architecture_image(self) -> str:
        """Obtiene referencia a la última imagen de arquitectura."""
        try:
